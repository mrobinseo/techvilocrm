...
export function AdSupportClient({ initialData, clients, currentUser, users }: AdSupportClientProps) {
  ...
  const totalUsd = data.reduce((sum, item) => sum + (Number(item.dollar_amount) || 0), 0)
  const totalBdt = data.reduce((sum, item) => sum + (Number(item.total_bdt) || 0), 0)
  const totalPaid = data.reduce((sum, item) => sum + (Number(item.paid_amount) || 0), 0)
  const totalDue = data.reduce((sum, item) => sum + (Number(item.due_amount) || 0), 0)
  ...
  async function handleDelete(id: string) {
    ...
    const res = await deleteAdSupport(id)
    if (res.success) {
