...
export default function ClientProfilePage() {
  ...
  const { clients, projects, ledgers, currentUser } = useStore()
  
  const client = clients.find(c => c.id === clientId)
  const isAdminOrManager = currentUser?.role === 'Admin' || currentUser?.role === 'Manager'
  const isOwner = (client as any)?.createdBy === currentUser?.id

  if (!client || (!isAdminOrManager && !isOwner)) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-zinc-400">
        <h2 className="text-xl font-bold text-white mb-2">
          {!client ? "Client Not Found" : "Access Denied"}
        </h2>
        <p className="mb-4">
          {!client 
        ...
        <Button onClick={() => router.push('/clients')} variant="outline" className="border-zinc-700 text-zinc-300">
          <ArrowLeft className="size-4 mr-2" /> Back to Clients
        </Button>
      </div>
    )
  }

  const clientProjects = projects.filter(p => p.client === client.company || p.client === client.name || p.client === 'Demo Client')
  const clientLedgers = ledgers.filter(l => l.clientId === clientId)

  const canSeeFinance = isAdminOrManager || isOwner

  const totalProjectValue = clientLedgers.reduce((sum, p) => sum + p.totalAmount, 0)
  const totalPaid = clientLedgers.reduce((sum, p) => sum + p.paidAmount, 0)
  const totalDue = clientLedgers.reduce((sum, p) => sum + p.dueAmount, 0)
                ...
                {clientProjects.map(project => (
                  <div key={project.id} className="flex flex-col gap-2 p-3 rounded-lg border border-zinc-800 bg-zinc-950/30">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      {canSeeFinance && <span className="font-medium text-rose-400">Due: ৳ {project.dueAmount.toLocaleString()}</span>}
                    </div>
                  </div>
                ))}
              </div>
                    ...
                    <TableBody>
                      {clientLedgers.map(ledger => (
                        <TableRow key={ledger.id} className="border-zinc-800 hover:bg-zinc-800/50">
                          <TableCell className="text-zinc-300">{ledger.paymentMonth}</TableCell>
                          <TableCell className="text-zinc-300 truncate max-w-[120px]">{ledger.project}</TableCell>
                          <TableCell className="text-right font-medium text-rose-400">
                            {ledger.dueAmount > 0 ? `৳ ${ledger.dueAmount.toLocaleString()}` : 'Paid'}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
