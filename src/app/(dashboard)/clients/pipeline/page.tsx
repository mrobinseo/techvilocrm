...
import { ArrowLeft, User, Mail, Phone, GripVertical } from "lucide-react"

export default function PipelinePage() {
  const router = useRouter()
  const { clients, updateClientStatus } = useStore()
  
  const leads = clients.filter(c => c.status === 'Lead')
  const active = clients.filter(c => c.status === 'Active')
  const inactive = clients.filter(c => c.status === 'Inactive')

  const moveStatus = (clientId: string, newStatus: string) => {
    updateClientStatus(clientId, newStatus)
  }

  const handleDragStart = (e: React.DragEvent, clientId: string) => {
    e.dataTransfer.setData("clientId", clientId)
    // Optional: Make the drag image look better or add styling
  ...
  // Reusable Card Component for the board
  const KanbanCard = ({ client, colorClass }: { client: any, colorClass: string }) => (
    <Card 
      draggable
      onDragStart={(e) => handleDragStart(e, client.id)}
      onDragEnd={handleDragEnd}
      className={`bg-zinc-900 border-zinc-800 border-l-4 ${colorClass} shadow-md cursor-grab active:cursor-grabbing hover:bg-zinc-800/50 transition-colors group`}
    >
      <CardContent className="p-3">
        <div className="flex items-start justify-between mb-1.5">
          ...
          <GripVertical className="size-3.5 text-zinc-600 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
        </div>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 h-[calc(100vh-6rem)] flex flex-col">
      <div className="flex items-center gap-4 shrink-0">
        ...
        <div>
          ...
          <p className="text-zinc-400">Drag and drop clients to update their status.</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3 flex-1 min-h-0 overflow-hidden">
        ...
        >
          ...
          <div className="space-y-2.5 overflow-y-auto flex-1 pr-1 custom-scrollbar pb-10">
            {inactive.map(client => (
              <KanbanCard key={client.id} client={client} colorClass="border-l-zinc-600 opacity-75" />
            ))}
            {inactive.length === 0 && (
              <div className="h-24 border-2 border-dashed border-zinc-800 rounded-lg flex items-center justify-center text-zinc-500 text-sm">
