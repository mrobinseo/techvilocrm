...
export function PaymentsClient({ 
  ...
  const handleEditPaymentSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await updatePayment(editLedgerId, formData)
  ...
  const handleDeletePaymentSubmit = async () => {
    if (deleteLedgerId) {
      setIsLoading(true)
      try {
        await deletePayment(deleteLedgerId)
  ...
  const handleExportCSV = () => {
    ...
    link.href = url
    link.download = `techvilo_ledger_${selectedClient.company}_${new Date().toISOString().split('T')[0]}.csv`
    link.click()
